...
//
// Monte Carlo Method usinng antithetic variates.
//
...
int main()
{
	...
	double price_2 = 0.0;	// Option price
	double finalPrice;
	...
	{ // Calculate a path at each iteration
		...
		for (long index = 1; index < x.size(); ++index)
		{
			...
			VNew_2 = VOld_2  + (k * drift(x[index-1], VOld_2))
						- (sqrk * diffusion(x[index-1], VOld_2) * dW);
		...
		price = 0.5*(myOption.myPayOffFunction(VNew) + myOption.myPayOffFunction(VNew_2));
	...
	finalPrice = sum_CT*exp(-myOption.r*myOption.T) /double(NSim);
