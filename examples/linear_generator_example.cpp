...
// can be any invertible_linear_generator
...
// any random distribution satisfying the C++11 standard (or possibly less)
...
int main(int argc, char * argv[])
{
	...
	cout << "Some raw random numbers from the " 
		 << mpl::c_str<Engine::name>::value << " engine:" << endl;
	generate_n( ostream_iterator<Engine::result_type>(cout, "\n"), 
				num_raw, eng );
	cout << endl;
	...
	cout << "Some random univariate numbers:" << endl;
	cout.precision( numeric_limits<Distribution::result_type>::digits10 );
	generate_n( ostream_iterator<Distribution::result_type>(cout, "\n"),
	...
	cout << "Checking whether jump matrix for jump size " 
		 ...
		 << " exists ... ";
	bool exists = Engine::JumpMatrix_file_exists(step_size - 1);
	cout << (exists ? " yes." 
	...
	if (Engine_is_reversible)
	{
		...
		if ( !Engine::JumpMatrix_file_exists(reverse_jump_size + 1, true) )
		{
	...
	if (computed_jump_matrix)
		cout << "Excecution will be rapid on the next run, because the " \
